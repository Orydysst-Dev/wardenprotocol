...
func MigrateStore(ctx sdk.Context, storeService store.KVStoreService, cdc codec.BinaryCodec) error {
	if err := migrateParams(ctx, storeService, cdc); err != nil {
		return err
	}
...
func migrateParams(ctx sdk.Context, storeService store.KVStoreService, cdc codec.BinaryCodec) error {
...
func migrate(ctx sdk.Context, ns NewStore, os OldStore) error {
	...
	ctx.Logger().Info("migrating action by address")
	actsByAddress, err := os.ActionByAddress.Iterate(ctx, nil)
	if err != nil {
		return err
	}
	for ; actsByAddress.Valid(); actsByAddress.Next() {
		kv, err := actsByAddress.KeyValue()
		if err != nil {
			return err
		}
		ctx.Logger().Info("migrating action by address", "address", kv.Key.K1(), "action_id", kv.Key.K2())
		if err := ns.ActionByAddress.Set(ctx, kv.Key, kv.Value); err != nil {
			return err
		}
	}
	ctx.Logger().Info("done migrating action by address")
